...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        ...
        if cls == target_y:
            break
        # print(scores.shape, scores[:, target_y].shape)
        loss = scores[0, target_y]
        loss.backward()
        g = X_adv.grad.data
        dX = learning_rate * g / g.norm(p=2)
        X_adv.data += dX.data
        print('Iteration %d: target score %.3f, max score %.3f' % (i, loss, scores[:, cls]))
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return X_adv
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    scores = model(img)
    # print(scores.shape, scores[:, target_y].shape)
    loss = scores[0, target_y] - l2_reg * img.norm(p=2) ** 2
    loss.backward()
    g = img.grad.data
