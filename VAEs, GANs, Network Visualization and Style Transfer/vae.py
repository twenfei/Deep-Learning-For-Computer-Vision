...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.encoder = nn.Sequential(
            nn.Flatten(), # start_dim=1 (default)
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        x_c = torch.cat((x.flatten(start_dim=1), c), dim=1)
        x_encoded = self.encoder(x_c)
        mu = self.mu_layer(x_encoded)
        logvar = self.logvar_layer(x_encoded)
        z = reparametrize(mu, logvar)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    rectr_loss = F.binary_cross_entropy(
        input=x_hat, target=x, reduction='sum'
    )
    kl_loss = - (1 + logvar - mu ** 2 - logvar.exp()).sum() / 2
    loss = (rectr_loss + kl_loss) / x.shape[0]
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
    return loss
