...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = torch.zeros((N, F, H1, W1), device=x.device, dtype=x.dtype)
        ...
        for n in range(N):
            for f in range(F):
                for i in range(H1):
                    for j in range(W1):
                        x_in = x_pad[n, :, stride*i: stride *
                                     i + HH, stride*j: stride*j  + WW]
                        out[n, f, i, j] = torch.sum(x_in * w[f]) + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
            ...
            for f in range(F):
                ...
                for i in range(H1):
                    for j in range(W1):
                        x_in = x_pad[n, :, stride*i: stride *
                                     i+HH, stride*j: stride*j+WW]
                        dw[f] += dout[n, f, i, j] * x_in
                        dx_pad[n, :, stride*i: stride*i+HH, stride *
                               j: stride*j+WW] += dout[n, f, i, j] * w[f]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros((N, C, H1, W1), device=x.device, dtype=x.dtype)
        for n in range(N):
            for c in range(C):
                for i in range(H1):
                    for j in range(W1):
                        out[n, c, i, j] = x[n, c, stride*i: stride*i +
                                            pool_height, stride*j: stride*j+pool_width].max()
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
            for c in range(C):
                for i in range(H1):
                    for j in range(W1):
                        x_in = x[n, c, stride*i: stride*i +
                                 pool_height, stride*j:stride*j+pool_width]
                        max_idx = x_in.argmax()
                        dx[n, c, stride*i+max_idx//pool_width, stride *
                            j+max_idx % pool_width] += dout[n, c, i, j]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W1'] = weight_scale * torch.randn(
            num_filters, C, filter_size, filter_size, device=device, dtype=dtype)
        self.params['b1'] = torch.zeros(
            (num_filters,), device=device, dtype=dtype)

        # input dim: (num_filters, H//2, W//2)
        # output dim: (hidden_dim,)
        # linear - relu
        self.params['W2'] = weight_scale * torch.randn(num_filters * (
            H//2) * (W//2), hidden_dim, device=device, dtype=dtype)
    ...
    def loss(self, X, y=None):
        ...
        dx3, dw3, db3 = Linear.backward(dout, cache3)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(1, self.num_layers):
            if self.batchnorm:
                self.params['gamma'+str(i)] = torch.ones(filters[i],
                                                         device=device, dtype=dtype)
                self.params['beta'+str(i)] = torch.zeros(filters[i],
                                                         device=device, dtype=dtype)
            if weight_scale == 'kaiming':
                self.params['W'+str(i)] = kaiming_initializer(filters[i-1],
                                                              filters[i], K=3, device=device, dtype=dtype)
            else:
                self.params['W'+str(i)] = weight_scale * torch.randn(filters[i],
                                                                     filters[i-1], 3, 3, device=device, dtype=dtype)
            self.params['b'+str(i)] = torch.zeros(filters[i],
                                                  device=device, dtype=dtype)
            # init last layer
            out_dim = filters[-1] * \
                (H // (2 ** len(max_pools))) * (W // (2**len(max_pools)))
            if weight_scale == 'kaiming':
                self.params['W'+str(self.num_layers)] = kaiming_initializer(
                    out_dim, num_classes, relu=False, device=device, dtype=dtype)
            else:
                self.params['W'+str(self.num_layers)] = weight_scale * torch.randn(out_dim,
                                                                                   num_classes, device=device, dtype=dtype)
            self.params['b' + str(self.num_layers)
                        ] = torch.zeros(num_classes, device=device, dtype=dtype)
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.batchnorm:
                ...
                # no batchnorm
                if i-1 in self.max_pools:
                    # print(i, w.shape, b.shape, x.shape)
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims=data_dict['X_train'].shape[1:],
      ...
      dtype=dtype,
      device=device)
    solver = Solver(model, data_dict, num_epochs=16, batch_size=256, print_every=1000, update_rule=adam,
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N = dout.shape[0]
        x_hat, gamma = cache['x_hat'], cache['gamma']
        dbeta = torch.sum(dout, dim=0)
        dgamma = torch.sum(dout * x_hat, dim=0)
        if cache['mode'] == 'train':
            dxhat = gamma * dout
            x, x_var, x_mean, eps = cache['x'], cache['x_var'], cache['x_mean'], cache['eps']
            in_std = torch.pow(x_var+eps, -0.5)
            din_std = torch.sum(dxhat * (x-x_mean), dim=0)
            dx_mu_1 = in_std * dxhat
            dvar = -0.5 * torch.pow(x_var+eps, -1.5) * din_std
            dx_mu_2 = 2.0 * (x - x_mean) * (torch.ones_like(dout)/N*dvar)
            dx1 = dx_mu_1 + dx_mu_2

            dmu = -torch.sum(dx1, dim=0)
            dx2 = torch.ones_like(dout) / N * dmu
            dx = dx1 + dx2
        else:
            x_var, eps = cache['x_var'], cache['eps']
            dx = gamma * dout * torch.pow(x_var+eps, -0.5)
    ...
    def backward_alt(dout, cache):
        ...
        # Replace "pass" statement with your code
        N = dout.shape[0]
        x_var, x_mean, x_hat, x = cache['x_var'], cache['x_mean'], cache['x_hat'], cache['x']
        ...
        if cache['mode'] == 'train':
            dxhat = gamma * dout
            dvar = torch.sum(dxhat * (x-x_mean) * (-0.5) * \
                torch.pow(x_var+eps, -1.5), dim=0)
            dmean = torch.sum(-dxhat / torch.sqrt(x_var+eps), dim=0) + \
                dvar * torch.mean(-2 * (x-x_mean), dim=0)
            dx = dxhat / torch.sqrt(x_var+eps) + 2 * \
                dvar / N * (x-x_mean) + 1.0/N*dmean
        else:
            dx = gamma * dout * torch.pow(x_var+eps, -0.5)
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        x_in = x.permute(0, 2, 3, 1).reshape(-1, C)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        dout_in = dout.permute(0, 2, 3, 1).reshape(-1, C)
        dx, dgamma, dbeta = BatchNorm.backward_alt(dout_in, cache)
