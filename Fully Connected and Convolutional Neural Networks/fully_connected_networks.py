...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        hidden, cache1 = Linear_ReLU.forward(
        ...
        loss += (torch.sum(w1 * w1) + torch.sum(w2 * w2)) * self.reg
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers+1):
            ...
            b = "b" + str(i)
            # print(w, b)
            self.params[w] = weight_scale * torch.randn(
                layer_dims[i-1], layer_dims[i], dtype=dtype, device=device
            )
            self.params[b] = torch.zeros(
                layer_dims[i], dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            x, cache = Linear_ReLU.forward(x, w, b)
            caches.append(cache)
            if self.use_dropout:
                x, dcache = Dropout.forward(x, self.dropout_param)
        ...
        grads['W'+str(self.num_layers)] = dw + w * self.reg
        grads['b'+str(self.num_layers)] = db
        for i, cache in reversed(list(enumerate(caches))):
            if self.use_dropout:
              dx = Dropout.backward(dx, dcaches.pop())
            dx, dw, db = Linear_ReLU.backward(dx, cache)
            (_, w, _), relu_cache = cache
            loss += torch.sum(w * w) * self.reg * 0.5
            grads["W" + str(i+1)] = dw + w * self.reg
            grads["b" + str(i+1)] = db
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def adam(w, dw, config=None):
    ...
    next_w = w - config['learning_rate'] * m_t / \
        (v_t.sqrt() + config['epsilon'])
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand_like(x,device=x.device) < (1.0 - p)).type(torch.float32)
            mask /= 1.0 - p
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
