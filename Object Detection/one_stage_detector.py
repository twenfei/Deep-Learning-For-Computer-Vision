...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        sz = len(stem_channels)
        input_channel = in_channels
        for i in range(0, sz):
          conv_cls = nn.Conv2d(input_channel, stem_channels[i], kernel_size=3, padding=1)
          conv_box = nn.Conv2d(input_channel, stem_channels[i], kernel_size=3, padding=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for i in range(3, 6):
            level_name = "p" + str(i)
            cls_hidden = feats_per_fpn_level[level_name].clone()
            box_hidden = feats_per_fpn_level[level_name].clone()
            B, _, H, W = cls_hidden.shape

            cls_hidden = self.stem_cls(cls_hidden)
            
            # class_logits_per_layer = self.pred_cls(cls_hidden) # (B, num_classes, H, W)
            class_logits[level_name] = self.pred_cls(cls_hidden).flatten(start_dim=2).permute(0, 2, 1)
            # class_logits[level_name] = self.pred_cls(cls_hidden).reshape(B, H*W, -1)

            box_hidden = self.stem_box(box_hidden)
            # boxreg_deltas_per_level = self.pred_box(box_hidden) # (B, 4, H, W)
            # centerness_logits_per_level = self.pred_ctr(box_hidden) # (B, 1, H, W)
            boxreg_deltas[level_name] = self.pred_box(box_hidden).flatten(start_dim=2).permute(0, 2, 1)
            centerness_logits[level_name] = self.pred_ctr(box_hidden).flatten(start_dim=2).permute(0, 2, 1)

            # print(class_logits[level_name].shape, boxreg_deltas[level_name].shape, centerness_logits[level_name].shape)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    N, M = gt_boxes.shape
    deltas = torch.zeros(N, 4, device=gt_boxes.device)
    # print(deltas.shape, locations.shape, gt_boxes.shape)
    ...
    background = (gt_boxes == torch.tensor([-1], device=gt_boxes.device)).all(dim=1)
    deltas[background, :] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    tmp_deltas = deltas.clone()
    tmp_deltas[tmp_deltas < 0] = 0
    N, _ = output_boxes.shape
    output_boxes[:, 0] = locations[:, 0] - tmp_deltas[:, 0] * stride
    output_boxes[:, 1] = locations[:, 1] - tmp_deltas[:, 1] * stride
    output_boxes[:, 2] = locations[:, 0] + tmp_deltas[:, 2] * stride
    output_boxes[:, 3] = locations[:, 1] + tmp_deltas[:, 3] * stride
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    N, _ = deltas.shape
    background = (deltas == torch.tensor([-1], device=deltas.device)).all(dim=1)
    centerness = torch.zeros(N, )
    lr_min = torch.minimum(deltas[:, 0], deltas[:, 2])
    tb_min = torch.minimum(deltas[:, 1], deltas[:, 3])
    lr_max = torch.maximum(deltas[:, 0], deltas[:, 2])
    tb_max = torch.maximum(deltas[:, 1], deltas[:, 3])
    centerness = (lr_min * tb_min / (lr_max * tb_max)).sqrt()
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        shape_per_fpn_level = {
            level_name: level_feat.shape
            for level_name, level_feat in fpn_feats.items()}
        ...
        for i in range(B):
            ...
            matched_gt_box_per_img = fcos_match_locations_to_gt(
                locations_per_fpn_level, strides_per_fpn_level, gt_boxes_per_img
            )
            matched_gt_boxes.append(matched_gt_box_per_img)
        ...
        for i in range(B):
            ...
            for level_name in ["p3", "p4", "p5"]:
                gt_deltas_per_img[level_name] = fcos_get_deltas_from_locations(
                    locations=locations_per_fpn_level[level_name], 
                    gt_boxes=matched_gt_boxes[i][level_name], 
                    stride=strides_per_fpn_level[level_name]
                )
            matched_gt_deltas.append(gt_deltas_per_img)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # Replace "pass" statement with your code
        # Compute loss_cls
        gt_classes = matched_gt_boxes[:, :, 4] + 1

        gt_classes_one_hot = F.one_hot(gt_classes.long(), self.num_classes + 1)[:, :, 1:]

        # print(gt_classes_one_hot.shape, pred_cls_logits.shape)
        loss_cls = sigmoid_focal_loss(
            inputs=pred_cls_logits, targets=gt_classes_one_hot.float()
        )
        # No loss for background:
        loss_cls[gt_classes < 0] *= 0.0

        # Compute loss_box
        # matched_gt_deltas = matched_gt_deltas.to(device=pred_boxreg_deltas.device)
        # print(matched_gt_deltas.device, pred_boxreg_deltas.device)
        ...
        ).reshape(pred_ctr_logits.shape)

        # print(pred_ctr_logits.shape, matched_gt_centerness.shape)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            level_pred_scores, level_pred_classes = \
                level_pred_scores.max(dim=1)

            # Step 2:
            # no background prediction of -1
            keep = (level_pred_classes == -1) & (level_pred_scores > test_score_thresh)
            ...
            level_pred_boxes[:, 2] = level_pred_boxes[:, 2].clamp(max=W-1)
            level_pred_boxes[:, 3] = level_pred_boxes[:, 3].clamp(max=H-1)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
        ...
        # print(keep.shape, pred_boxes_all_levels.shape)
