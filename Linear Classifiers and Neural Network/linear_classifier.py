...
def svm_loss_vectorized(
    ...
    margin = scores - correct_class_score.view(-1, 1) + 1  # note delta = 1
    margin = torch.maximum(margin, torch.zeros_like(margin))
    margin[range(num_train), y] = 0

    # Average out the loss on training examples
    loss = margin.sum(dim=1).mean()
    # Add regularization to the loss.
    loss += reg * torch.sum(W * W)
    ...
    # For y[i] != j
    # mask = margin.clone()
    margin[margin > 0] = 1  # mask where y[i] != j and margin > 0
    class_sum = margin.sum(dim=1)
    margin[range(num_train), y] = -class_sum.t()
    dW = X.t().mm(margin)
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            class_sum += torch.exp(scores[j])

        # update loss
        loss += -scores[y[i]] + torch.log(class_sum)
        # update gradient
        dW[:, y[i]] -= X[i]
        for j in range(num_classes):
            dW[:, j] += (torch.exp(scores[j])/class_sum) * X[i]
...
def softmax_loss_vectorized(
    ...
    class_sum = torch.sum(scores.exp(), dim=1)  # (N, 1)

    loss += -scores[range(num_train), y].sum() + class_sum.log().sum()

    params = torch.zeros_like(scores) # (N, D)
    params[range(num_train), y] = -1
    # print(scores.shape, class_sum.shape, params.shape)
    params += scores.exp() / class_sum.view(-1, 1)
    dW = X.t().mm(params)
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-2, 1e-1, 3e-1]
    regularization_strengths = [1e-2, 5e-3, 1e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
