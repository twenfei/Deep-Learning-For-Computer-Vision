...
def nn_forward_backward(
    ...
    class_sum = torch.sum(scores.exp(), dim=1)
    loss += -scores[range(batch_size), y].sum() + class_sum.log().sum()

    loss /= batch_size
    loss += torch.sum(W1 * W1) * reg
    loss += torch.sum(W2 * W2) * reg
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    # Replace "pass" statement with your code
    # TODO
    dL = torch.zeros_like(scores)  # (N, C)
    dL[range(batch_size), y] = -1
    dL += scores.exp() / class_sum.view(-1, 1)

    grads['W2'] = h1.t().mm(dL) / batch_size  # (H, C)
    grads['b2'] = dL.sum(dim=0) / batch_size
    grads['W2'] += 2 * W2 * reg

    dh1 = dL.mm(W2.t())  # (N, H)
    dh1[h1 == 0] = 0

    grads['W1'] = X.t().mm(dh1) / batch_size  # (D, H)
    grads['b1'] = dh1.sum(dim=0) / batch_size
    grads['W1'] += 2 * W1 * reg
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
            ...
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    classifier = TwoLayerNet(
                        input_size, hidden_sz, 10)
                    cand_stat = classifier.train(X_train, y_train, X_val,
                                                 y_val, lr, lr_decay, reg, num_iters=2000)
                    ...
                    #print('Current Validation Acc: ', val_acc)
                    #print('Params: ', lr, lr_decay, reg, hidden_sz)
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
