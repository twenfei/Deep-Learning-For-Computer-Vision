...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        ...
        test_flat = x_test[j].flatten()
        dists[i, j] = (train_flat - test_flat).pow(2).sum()
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      dists[i, :] = (test_flat - train_flat[i]).pow(2).sum(dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # sum((xi - yj) ^ 2) = sum(xi^2) + sum(yj^2) - sum(2xiyj)
    x_2 = x_train.flatten(start_dim=1).pow(2).sum(dim=1).view(-1, 1)
    y_2 = x_test.flatten(start_dim=1).pow(2).sum(dim=1).view(1, -1)
    dists = x_2 + y_2 - 2 * x_train.flatten(start_dim=1).matmul(x_test.flatten(start_dim=1).t())
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      _, top_k = torch.topk(dists[:, i], k, dim=0, largest=False)
      y_pred[i], _ = torch.mode(y_train[top_k])
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        ...
        train_label = torch.cat((*y_train_folds[:i], *y_train_folds[i+1:]))

        test_data = x_train_folds[i]
        test_label = y_train_folds[i]

        knn = KnnClassifier(train_data, train_label)
        acc = knn.check_accuracy(test_data, test_label, k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for key in sorted(k_to_accuracies):
      mean_acc = sum(k_to_accuracies[key]) / len(k_to_accuracies[key])
