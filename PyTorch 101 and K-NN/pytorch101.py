...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = torch.zeros((x.shape[0], x.shape[1]), dtype=x.dtype)
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    y = torch.clone(x)
    y = y.view(2, 3, 4)
    y = y.transpose(0, 1).contiguous().view(3, 8) # TODO
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.clone(x)
    col_min_idxs = y.min(dim=1)[1]
    row_idxs = torch.arange(y.shape[0])
    y[row_idxs, col_min_idxs] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    z = torch.stack([x[d] @ y[d] for d in range(x.shape[0])])
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    m = x.shape[0]
    n = x.shape[1]
    mu_j = (torch.sum(x, dim=0)) / m # compute mu
    x_mu = torch.sum((x - mu_j) ** 2, dim=0)
    sigma_j = torch.sqrt(1/(m-1) * x_mu)
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    padded_tensor = torch.nn.utils.rnn.pad_sequence(xs)
    tensor_sum = torch.sum(padded_tensor, dim=0)
    y = torch.div(tensor_sum, torch.tensor(ls))
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    sorted, idx = torch.sort(x, stable=True)
    uniques, counts = torch.unique_consecutive(sorted, return_counts=True)
    sorted_indices = torch.cumsum(counts, dim=0).roll(shifts=1)
    sorted_indices[0] = 0
    indices = idx[sorted_indices]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return uniques, indices
