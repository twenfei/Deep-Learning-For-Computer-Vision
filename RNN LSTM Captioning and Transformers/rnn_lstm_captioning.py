...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # print(prev_h.dtype, Wh.dtype, x.dtype, Wx.dtype)
    next_h = prev_h.matmul(Wh) + x.matmul(Wx) + b
    next_h = torch.tanh(next_h)
    cache = (x, prev_h, Wx, Wh, b, next_h) # maybe we don't need all of them
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    # next_h: (N, H), dnext_h: (N, H), dtanh: (N, H)
    dtanh = (1 - next_h ** 2) * dnext_h
    # dx: (N, D), dtanh: (N, H), Wx: (D, H)
    dx = dtanh.mm(Wx.t())
    # dprev_h: (N, H), dtanh: (N, H), Wh:(H, H)
    dprev_h = dtanh.mm(Wh.t())
    # dWx: (D, H), dtanh: (N, H), x: (N, D)
    dWx = x.t().mm(dtanh)
    # dWh: (H, H), dtanh: (N, H), prev_h: (N, H)
    dWh = prev_h.t().mm(dtanh)
    # db: (H,), dtanh: (N, H)
    db = dtanh.sum(dim=0).view(-1)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1 , -1):
        dnext_h = dh[:, t, :] + dh0
        cache_t = cache.pop()
        dx[:, t, :], dh0, dWxt, dWht, dbt = rnn_step_backward(dnext_h, cache_t)
        dWx += dWxt
        dWh += dWht
        db += dbt
    ##########################################################################
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    loss = F.cross_entropy(
        input=x.flatten(start_dim=0, end_dim=1),
        target=y.flatten(), 
        ignore_index=ignore_index, 
        reduction="sum"
    )   
    loss /= x.shape[0] # averaging across the minibatch
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.feat_layer = nn.Linear(
            in_features=self.image_encoder.out_channels,
            out_features=hidden_dim
            # dtype=torch.float64
        )

        self.pooling_layer = torch.nn.AvgPool2d(kernel_size=(4, 4))
        self.affine_att = None
        if cell_type == "attn":
            self.affine_att = nn.Conv2d(
        ...
        rnn_box = None
        if cell_type == "rnn":
            rnn_box = RNN(wordvec_dim, hidden_dim)
        elif cell_type == "lstm":
            rnn_box = LSTM(wordvec_dim, hidden_dim)
        elif cell_type == "attn":
            rnn_box = AttentionLSTM(wordvec_dim, hidden_dim)
        self.rnn_box = rnn_box
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "attn":
            A = self.affine_att(feats)
        else:
            pooled_feats = self.pooling_layer(feats).flatten(start_dim=1)
            h0 = self.feat_layer(pooled_feats)
        # Step(2)
        embedded_captions_in = self.word_embedding(captions_in) # (N, T, W)
        # Step(3)
        hidden_states = None
        if self.cell_type == "attn":
            hidden_states = self.rnn_box(embedded_captions_in, A)
        else:
            hidden_states = self.rnn_box(embedded_captions_in, h0) # (N, T, H)
        # Step(4)
        # scores = self.output_layer(hidden_states.to(torch.float32)) # (N, T, V)
        scores = self.output_layer(hidden_states) # (N, T, V)
        # Step(5)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "attn":
            ...
            pooled_feats = self.pooling_layer(feats).flatten(start_dim=1)
            prev_h = self.feat_layer(pooled_feats)
            if self.cell_type == "lstm":
                prev_c = torch.zeros_like(prev_h, device=prev_h.device)
        prev_emb = self.word_embedding(self._start)
        

        for i in range(max_length):
            if self.cell_type == "rnn":
                next_h = self.rnn_box.step_forward(prev_emb, prev_h) # (N, H)
            elif self.cell_type == "lstm":
                next_h, next_c = self.rnn_box.step_forward(prev_emb, prev_h, prev_c)
                prev_c = next_c
            else:
                attn, attn_weights = dot_product_attention(prev_h, A)
                next_h, next_c = self.rnn_box.step_forward(prev_emb, prev_h, prev_c, attn)
                prev_c = next_c
            prev_h = next_h
            # scores = self.output_layer(next_h.to(torch.float32)) # (N, V)
            scores = self.output_layer(next_h) # (N, V)
            next_word = torch.argmax(scores, dim=1) # (N, )
            captions[:, i] = next_word
            prev_emb = self.word_embedding(next_word)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        if self.cell_type == "attn":
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        next_h = torch.zeros_like(prev_h, dtype=x.dtype)
        next_c = torch.zeros_like(prev_c, dtype=x.dtype)
        H = self.Wh.shape[0]
        a = x.matmul(self.Wx) + prev_h.matmul(self.Wh) + self.b # (N, 4H)
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    A_flat = A.flatten(start_dim = 2) # (N, H, 16)
    attn = torch.zeros(N, H, device=prev_h.device) # (N, H)
    attn_weights = torch.zeros(N, 4, 4, device=prev_h.device) # (N, 4, 4)

    prev_h = prev_h.reshape((N, 1, H))
    M_attn = (torch.bmm(prev_h, A_flat)/ (math.sqrt(H))).reshape((N, 16)) # (N, 1, 16)
    M_attn = F.softmax(M_attn, dim=1) # (N, 16)

    attn_weights = M_attn.reshape(N, 4, 4)
    attn = torch.bmm(M_attn.reshape((N, 1, 16)), A_flat.permute(0, 2, 1)).reshape(N, H)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        next_h = torch.zeros_like(prev_h, dtype=x.dtype)
        next_c = torch.zeros_like(prev_c, dtype=x.dtype)
        # print(next_h.dtype) # torch.float64
        H = self.Wh.shape[0]
        a = x.matmul(self.Wx) + prev_h.matmul(self.Wh) + attn.matmul(self.Wattn) + self.b # (N, 4H)
        i = torch.sigmoid(a[:, 0: H])
        f = torch.sigmoid(a[:, H: 2 * H])
        o = torch.sigmoid(a[:, 2 * H: 3 * H])
        g = torch.tanh(a[:, 3 * H:])
