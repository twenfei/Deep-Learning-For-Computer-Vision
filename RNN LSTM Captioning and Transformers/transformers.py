...
def scaled_dot_product_two_loop_single(
    ...
    args:
        query: a Tensor of shape (K1, M) where K is the sequence length and M is
            the sequence embeding dimension

        key: a Tensor of shape (K2, M) where K is the sequence length and M is the
            sequence embeding dimension

        value: a Tensor of shape (K2, M) where K is the sequence length and M is
            the sequence embeding dimension


    Returns
        out: a tensor of shape (K1, M) which is the output of self-attention from
        the function
    """
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    product = torch.zeros(query.shape[0], key.shape[0], device=query.device, dtype=query.dtype)
    for q_id in range(query.shape[0]):
        for k_id in range(key.shape[0]):
            product[q_id, k_id] = torch.dot(query[q_id], key[k_id])
    product /= query.shape[1] ** 0.5 # (K1, K2)
    weights = torch.softmax(product, dim=1)
    out = weights.matmul(value) # (K1, K2), (K2, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    args:
        query: a Tensor of shape (N, K1, M) where N is the batch size, K is the
            sequence length and  M is the sequence embeding dimension

        key: a Tensor of shape (N, K2, M) where N is the batch size, K is the
            sequence length and M is the sequence embeding dimension


        value: a Tensor of shape (N, K2, M) where N is the batch size, K is the
            sequence length and M is the sequence embeding dimension


    Returns:
        out: a tensor of shape (N, K1, M) that contains the weighted sum of values
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    product = torch.zeros((N, query.shape[1], key.shape[1]), device=query.device)
    for i in range(query.shape[1]):
        q = query[:, i, :].view(N, 1, -1)
        for j in range(key.shape[1]):
            k = key[:, j, :].view(N, -1, 1)
            # batch-wise dot product
            product[:, i, j] = q.bmm(k).squeeze()
    product = product / (M**0.5)
    weights = torch.softmax(product, dim=2)
    out = weights.bmm(value) # (N, K1, K2) * (N, K2, M) = (N, K1, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        y, self.weights_softmax = \
            scaled_dot_product_no_loop_batch(Q, K, V, mask)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.back_map = nn.Linear(dim_out * num_heads, dim_in)
        c = (6 / (num_heads * dim_out + dim_in)) ** 0.5
        nn.init.uniform_(self.back_map.weight, a=-c, b=c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        y_lst = []
        for head in self.self_attentions:
            y_single = head(query, key, value, mask)
            y_lst.append(y_single)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        mlp_1 = nn.Linear(inp_dim, hidden_dim_feedforward)
        nn.init.uniform_(mlp_1.weight, a=-c, b=c)

        mlp_2 = nn.Linear(hidden_dim_feedforward, inp_dim)
        nn.init.uniform_(mlp_2.weight, a=-c, b=c)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out2 = self.layer_norm_1(out1 + x)
        out2 = self.dropout(out2)
        out3 = self.feed_forward(out2)
        out = self.layer_norm_2(out2 + out3)
        y = self.dropout(out)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
def get_subsequent_mask(seq):
    ...
    mask = []
    for i in range(K):
        mask.append([False] * (i + 1) + [True] * (K - i - 1))
    mask = torch.tensor(mask, device=seq.device).view(1, K, K)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    a = torch.floor(i / M)
    p = torch.arange(K).unsqueeze(-1) # (K, 1)
